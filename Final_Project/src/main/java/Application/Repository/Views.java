...
public class Views {
    ...
    /**
     * Este método, dada uma empresa, retorna a lista de representantes da mesma que interagiram
     * com outras empresas.
     *
     * @param driver  instância do driver para comunicar com a base de dados
     * @param company nome da empresa
     * @return os representantes da empresa
     */
    public static Map<String, Object> getCompanyRepresentants(Driver driver, String company) {
        try (Session session = driver.session()) {
            Map<String, Object> parameters = new HashMap<>();
            parameters.put("Company", company);

            Record queryResults = session.writeTransaction(tx -> tx.run(
                    "MATCH (com:" + Entities.Labels.Company + ")\n"
                            + "WHERE com.CompanyName = $Company\n"

                            + "OPTIONAL MATCH (c:" + Entities.Labels.Customer + ")-[:" + Entities.CustomerRelationships.REPRESENTS_AS_CUSTOMER + "]->(com)\n"
                            ...
                            + "WITH com, c, ac, ctid, company, contact, ba, babn, basn, bac, bapc, bar, bacountry, "
                            + "collect("
                            + "{"
                            ...
                            + "OPTIONAL MATCH (s:" + Entities.Labels.Supplier + ")-[:" + Entities.SupplierRelationships.REPRESENTS_AS_SUPPLIER + "]->(com)\n"
                            ...
                            + "WITH com, Customers, s, ac, stid, company, contact, ba, babn, basn, bac, bapc, bar, bacountry, "
                            + "collect("
                            + "{"
                            + "BuildingNumber: sabn.BuildingNumber, "
                            + "StreetName: sasn.StreetName, "
                            ...
                            + "RETURN com.CompanyName AS Company, Customers AS RepresentantsAsCustomer, Suppliers AS RepresentantsAsSupplier\n",
                    parameters
            ).single());

            return queryResults.asMap();
        }
    }
